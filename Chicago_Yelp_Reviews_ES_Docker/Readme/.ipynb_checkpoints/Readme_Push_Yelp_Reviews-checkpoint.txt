...
Requirements -> Python 3.11 and elasticsearch 8.12 and yelpapi, pandas, pyarrow
...
Step_2: Open the terminal and navigate to project directory and using the command below pull the original elasticsearch 8.12.0 image from docker hub:
...
Step_3: Make/run the python script for creating changes.
...
Step_6: Now we need to commit our changes, follow this to create commit command :
	 ...
	 For example : docker commit 6c31284597e6 prakharnag98/elasticsearch_8.12:latest

         This commits all the new changes and will create a new image.


Step_7: Now, push the Image to the Docker hub using the below command.

	docker push YOUR-DOCKER-USERNAME/[REPOSITORY[:TAG]]

	For example: docker push prakharnag98/elasticsearch_8.12:latest
...
Step_8: Now, lets build a multi-platform image and push it on docker hub
	...
	Step_8.1:     Open the given Dockerfile in text editor and make changes accordingly as shown below and save it. 
	     	      ...
	     	      FROM [REPOSITORY[:TAG]]
	      	      ...
	      	      For example : FROM prakharnag98/elasticsearch_8.12:latest
	...
	Step_8.3:     docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7 -t <username>/<image>:multi --push .
                      ...
                      For example: docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7 -t prakharnag98/elasticsearch_8.12:multi --push .
